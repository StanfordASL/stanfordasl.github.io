      ...
      {faculty.map((person) => (
        <div key={person.slug} className="mt-12 grid grid-cols-1 gap-12 lg:grid-cols-3">
          <div className="lg:col-span-1">
            {person.website ? (
              <a href={person.website} className="block aspect-square overflow-hidden rounded-xl shadow-xl outline-1 -outline-offset-1 outline-black/10">
                <img
                  alt={person.title}
  ...
  return (
    <Container className="mt-24">
      ...
      >
        {team.map((person) => (
          <li key={person.slug}>
            <div className="aspect-square w-full overflow-hidden rounded-xl outline-1 -outline-offset-1 outline-black/5">
              <img
                alt={person.title}
                src={`/_people/people-imgs/${person.img}`}
                className="block size-full object-cover"
            ...
            <div className="mt-4 flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {person.position === 'phd' ? 'PhD Student' : 'Postdoctoral Scholar'}
                </p>
              </div>
